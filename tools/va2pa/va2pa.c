...
static int find_pgd_init(void)
{
    ...
    printk(KERN_INFO "pte_val(*pte_tmp) = 0x%lx", pte_val(*pte_tmp));
    if (pte_none(*pte_tmp)) {
    ...
    printk(KERN_INFO "Virtual address 0x%lx in RAM is 0x%lx.\n", va, pa);

    // This will arise kernel paging error
    //printk(KERN_INFO "content in 0x%lx is 0x%lx\n", pa,
